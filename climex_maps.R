...
# Crop to exten of Australia
#r1 <- crop(r1, extent(112, 155, -45, -10))

#fishnet for table joins
fishnet <- readOGR("./data/fishnet_aus.shp")
...
spp1975 <- read.csv (paste0("/home/hil32c/Documents/climex_out/",spp,"_1975H.csv"))
...
gplot(spp1975R) +
  ...
  scale_fill_manual(values=c(Colors), labels=c("Baseline", "Increase", "Decrease"),
                    name= "Ecoclimatic Index")+
  labs(x = "Latitude", y="Longitude") +
  #scale_colour_manual(values = c("0" = "grey90", "1" = "black", "2" = "grey40"))+
  coord_equal()
...
setwd("C:\\Users\\hil32c\\Documents\\climex_out")

#spp.a1b.cs <- read.csv (paste0("./",spp,"_",year,"_A1B_CS.csv"))
#spp.a1b.mr <- read.csv (paste0("./",spp,"_",year,"_A1B_MR.csv"))
spp.a2.cs <- read.csv (paste0("./",spp,"_",year,"_A2_CS.csv"))
spp.a2.mr <- read.csv (paste0("./",spp,"_",year,"_A2_MR.csv"))

#spdf.a1b.cs<- merge(fishnet, spp.a1b.cs, by.x="Location")
#spdf.a1b.mr<- merge(fishnet, spp.a1b.mr, by.x="Location")
spdf.a2.cs<- merge(fishnet, spp.a2.cs, by.x="Location")
spdf.a2.mr<- merge(fishnet, spp.a2.mr, by.x="Location")

#spp.a1b.csR <- rasterize(spdf.a1b.cs, r1, field="EI", fun='last')
#spp.a1b.mrR <- rasterize(spdf.a1b.mr, r1, field="EI", fun='last')
spp.a2.csR <- rasterize(spdf.a2.cs, r1, field="EI", fun='last')
spp.a2.mrR <- rasterize(spdf.a2.mr, r1, field="EI", fun='last')

spp.cc.R <- spp.a2.csR + spp.a2.mrR
spp.cc.R <- spp.cc.R/2

spp1975R2 <- reclassify (spp1975R, c(-Inf, 10, 0, 10, Inf, 1))
spp.cc.R2 <- reclassify (spp.cc.R, c(-Inf, 10, 0, 10, Inf, 1))

maskR <-  spp.cc.R2
maskR <- reclassify (maskR, c(-Inf, 0, NA, 0, Inf, 1))

change.R <- spp.cc.R - spp1975R

#change.R <- mask (change.R, maskR)

r <- reclassify(change.R, c(0, Inf, 2, -Inf, 0,  3))
#r <- reclassify (r, c(-Inf, 0, 0))
p <- reclassify (spp1975R2 , c(0, Inf, 1))
r <- mask (r, maskR)
...
#pdf (file = paste0("./",spp,year,".pdf"), paper="special", width=15, height=8)
...
spp2050 <- read.csv (paste0("./",spp,"_2050_MR.csv"), skip=2)
spp2090 <- read.csv (paste0("./",spp,"_2050_MR.csv"), skip=2)
